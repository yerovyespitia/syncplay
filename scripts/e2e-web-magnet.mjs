...
if (!cdpPortArg || !magnetLink) {
  console.error("Usage: node scripts/e2e-web-magnet.mjs <cdp-port> <magnet-link>");
  process.exit(1);
}
...
async function main() {
  ...
  try {
    ...
    try {
      ...
      const screenshotDir = await fs.mkdtemp(path.join(os.tmpdir(), "syncplay-web-magnet-e2e-"));
