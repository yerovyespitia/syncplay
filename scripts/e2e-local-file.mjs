...
if (!cdpPortArg || !filePath) {
  console.error("Usage: node scripts/e2e-local-file.mjs <cdp-port> <file-path>");
  process.exit(1);
}
...
async function main() {
  ...
  try {
    ...
    try {
      ...
      const screenshotDir = await fs.mkdtemp(path.join(os.tmpdir(), "syncplay-e2e-"));
